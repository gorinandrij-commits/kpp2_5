...
async function createRoom() {
  ...
  try {
    ...
    if (data.room_id) {
      ...
      // тут потім можна this.fetchMessages();
...
// Завантаження кімнат + сортування
  ...
  try {
    ...
    if (data.joined_rooms) {
      const roomPromises = data.joined_rooms.map(async (roomId) => {
        ...
        return {
          roomId,
          name: (nameData && nameData.name) || this.getRoomName(roomId) || roomId
        };
      });

      // Сортування кімнат за roomId (як масив, що сортується)
      this.rooms = (await Promise.all(roomPromises))
        .sort((a, b) => a.roomId.localeCompare(b.roomId));

      if (this.rooms.length > 0 && !this.roomId) {
        this.roomId = this.rooms[0].roomId;
        // тут потім – this.fetchMessages();
      }
    }
  } catch (e) {
